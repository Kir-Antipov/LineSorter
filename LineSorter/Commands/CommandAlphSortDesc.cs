...
namespace LineSorter.Commands
{
    internal sealed class CommandAlphSortDesc : BaseCommand<CommandAlphSortDesc>
    {
        ...
        protected override void Execute(OleMenuCommand Button) =>
            TextSelection.GetSelection(Package, out int[] poses, out NewlineType newlineType, out bool newLine).Select(x => (Row)x).OrderByDescending(x => x.Cleared).ThenByDescending(x => x.Cleared.Length).Select(x => (string)x).ReplaceSelection(poses, newlineType, newLine);
        #endregion
    }
}
